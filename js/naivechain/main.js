// recreated from https://github.com/lhartikk/naivechain
// just didn't do docker as well
...
var handleBlockchainResponse = (message) => {
  var recievedBlocks = JSON.parse(message.data).sort((b1, b2) => (b1.index - b2.index));
  var latestBlockRecieved = recievedBlocks[recievedBlocks.length - 1];
  var latestBlockHeld = getLatestBlock();
  if(latestBlockRecieved.index > latestBlockHeld.index) {
    console.log('Blockchain possibly behind. We got: ' + latestBlockHeld.index + ' Peer got: ' + latestBlockRecieved.index);
    if(latestBlockHeld.hash === latestBlockRecieved.previousHash) {
      console.log('We can append the received block to our chain');
      blockchain.push(latestBlockRecieved);
      broadcast(responseLatestMsg());
